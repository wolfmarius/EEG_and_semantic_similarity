...
%load(strcat(path_table, selvps{n},'table_ngd'));  
load(strcat(path_table, selvps{n}, 'table_wv'));
...
for c_ind=1:size(cat_sparse,1) %for loop executed with parallel computing.    
               [corr_all{c_ind}, auto_trial{c_ind}]=vectorized_sliding_after(data, data_vec, c_ind, n_bins, cat_sparse, code_groups, code_blocks);  %kann ich durch eine korrelation ersetzen.               
end
...
save(strcat(path_out,selvps{n},'_slidRSA_steps',num2str(win*1000),'ms_slide',num2str(slide*1000),'ms_time',num2str(t_start),'_',num2str(t_end),'HZ100_hp30.mat'), 'tois','corr_all','cat_sparse','auto_trial','t1','t2','n_groups','code_groups', 'win', 'slide', 't_start', 't_end')
end
